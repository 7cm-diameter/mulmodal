...
async def control(agent: Agent, ino: Arduino, expvars: Experimental):
    ...
    overall_cue_duration: float = expvars.get("cue-duration", 2.)
    cue_interval: float = expvars.get("cue_interval", 1.)
    second_cue_duration = overall_cue_duration - cue_interval
    white_noise = make_white_noise(overall_cue_duration)
    ...
    waittime_limit: float = expvars.get("limit-of-waittaime", 3.)
    ...
    light_positions: list[int] = blockwise_shuffle(sum(repeat([light_pins],
                                                       ...
                                                       []),
                                                   len(light_pins))
    stimulus_sequence: list[int] = blockwise_shuffle(sum(repeat([[0, 1]],
                                                                [number_of_rewards // 2]),
                                                         []),
                                                     10)
    trials = TrialIterator2(isis, light_positions, stimulus_sequence)

    try:
        while agent.working():
            for i, isi, light, seq in trials:
                show_progress(i, isi)
                ...
                if seq == 0:
                    ...
                    await flush_message_for(agent, cue_interval)
                    agent.send_to(RECORDER, timestamp(light))
                    ino.digital_write(light, HIGH)
                    await fixed_interval_with_limit(agent, second_cue_duration, response_pins[0], postpone, waittime_limit)
                    agent.send_to(RECORDER, timestamp(-NOISE_IDX))
                    agent.send_to(RECORDER, timestamp(-light))
                    speaker.stop()
                    ...
                    agent.send_to(RECORDER, timestamp(NOISE_IDX))
                    speaker.play(white_noise, blocking=False, loop=True)
                    await fixed_interval_with_limit(agent, second_cue_duration, response_pins[1], postpone, waittime_limit)
                    agent.send_to(RECORDER, timestamp(-light))
                    agent.send_to(RECORDER, timestamp(-NOISE_IDX))
                    ino.digital_write(light, LOW)
                    speaker.stop()
